...
const GLOBAL_REDIS_CLIENT_KEY = "__neutralalphaTelemetryRedisClient";
const STATE_STORE_KEY = process.env.SIM_STATE_STORE_KEY ?? "neutralalpha:telemetry:state:v1";
...
function ensureStateShape(candidate) {
  ...
  if (!Array.isArray(state.navSeries) || !Array.isArray(state.fundingSeries)) {
    return null;
  }
  if (!state.liveFunding || typeof state.liveFunding !== "object") {
    return null;
  }
  ...
  if (!Array.isArray(state.riskAlerts)) {
    state.riskAlerts = [];
  }
  if (!Array.isArray(state.activity)) {
    state.activity = [];
  }
  if (!Number.isFinite(Number(state.lastTickMs))) {
    state.lastTickMs = Date.now();
  }
  if (typeof state.lastUpdate !== "string") {
    state.lastUpdate = new Date().toISOString();
  }
  if (typeof state.activeAsset !== "string") {
    state.activeAsset = "SOL";
  }
...
function getRedisClient() {
  const globalScope = globalThis;
  if (globalScope[GLOBAL_REDIS_CLIENT_KEY] !== undefined) {
    return globalScope[GLOBAL_REDIS_CLIENT_KEY];
  }

  const url = (process.env.KV_REST_API_URL ?? process.env.UPSTASH_REDIS_REST_URL ?? "").trim();
  const token = (process.env.KV_REST_API_TOKEN ?? process.env.UPSTASH_REDIS_REST_TOKEN ?? "").trim();
  if (!url || !token) {
    globalScope[GLOBAL_REDIS_CLIENT_KEY] = null;
    return null;
  }

  const client = new Redis({ url, token });
  globalScope[GLOBAL_REDIS_CLIENT_KEY] = client;
  return client;
}
...
async function loadState() {
  ...
  try {
    const raw = await redis.get(STATE_STORE_KEY);
    if (!raw) {
      const initialState = getMemoryStore();
    ...
    const shaped = ensureStateShape(parsed);
    if (shaped) {
      setMemoryStore(shaped);
      return shaped;
    }
  } catch {
    return getMemoryStore();
...
async function saveState(state) {
  ...
  try {
    await redis.set(STATE_STORE_KEY, JSON.stringify(state));
  } catch {
    // Fallback remains available in process memory.
  }
}
...
export async function getDashboardSnapshot() {
  ...
  const advanced = syncToNow(state);
  if (advanced) {
...
export async function applyRiskSimulation(payload) {
  const state = await loadState();
  syncToNow(state);
...
export async function applyDeposit(payload) {
  const state = await loadState();
  syncToNow(state);
...
export async function applyWithdraw(payload) {
  const state = await loadState();
  syncToNow(state);
  const amount = Number(payload?.amountUsd);
  const slippagePct = Number(payload?.slippagePct ?? 0);
