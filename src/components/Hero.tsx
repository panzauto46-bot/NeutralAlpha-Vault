...
import { NETWORK_LABEL } from "@/config/network";
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-16">
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div
          className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-blue-500/15 rounded-full blur-3xl animate-pulse-glow"
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        >
          ...
          <span className="text-sm text-green-400 font-medium">Live on Solana {NETWORK_LABEL}</span>
        ...
        <motion.p
          ...
          className="text-base text-slate-400 mb-10 max-w-2xl mx-auto leading-relaxed"
        >
          Capture funding spread without directional market exposure. The rebalancing engine
          adjusts positions every 15 minutes to keep risk constrained and yield stable.
        </motion.p>
        ...
        >
          {[
            ...
            { icon: Bot, label: "AI Rebalancing", color: "blue" },
            { icon: TrendingUp, label: "Funding Arbitrage", color: "green" },
            { icon: Zap, label: "Multi-Asset Rotation", color: "blue" },
          ].map((feature, i) => (
            <div
              key={i}
              className={`flex items-center gap-2 px-4 py-2 rounded-full glass border ${
                feature.color === "green" ? "border-green-500/20" : "border-blue-500/20"
              }`}
            >
              <feature.icon
                className={`w-4 h-4 ${feature.color === "green" ? "text-green-400" : "text-blue-400"}`}
              />
